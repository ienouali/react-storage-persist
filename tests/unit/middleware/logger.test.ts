...
describe('loggerMiddleware', () => {
  ...
  it('should not log get operations when disabled', async () => {
    ...
    expect(mockLogger.log).not.toHaveBeenCalledWith('[Storage] GET', expect.anything());
  });
  ...
  it('should use custom prefix', async () => {
    storage.use(loggerMiddleware({ logger: mockLogger, prefix: '[MyApp]', logSet: true }));

    await storage.set('key', 'value');

    expect(mockLogger.log).toHaveBeenCalledWith('[MyApp] SET', expect.anything());
  });
